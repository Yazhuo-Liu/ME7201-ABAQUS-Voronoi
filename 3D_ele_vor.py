...
# Analysis Part
myPart_ele = myModel.Part(name='Element_Vor', dimensionality=THREE_D, type=DEFORMABLE_BODY)
myPart_ele.BaseSolidExtrude(sketch=mySketch2, depth=depth)

myPart_ele.seedPart(size=min(length,width,depth)/50.0, deviationFactor=0.1, minSizeFactor=0.1)
myPart_ele.generateMesh()

myCells = myPart.cells
myNodes = myPart_ele.nodes
myElement = myPart_ele.elements


for i in range(len(myPart.cells)):
    # create material
    a,b,c = np.random.uniform(0,1),np.random.uniform(0,1),np.random.uniform(0,1) 
...
for set_element in element_number:
    if set_element == []:
        ...
        region = myPart_ele.Set(elements=element, name="Set-{}".format(element_number.index(set_element)))
        p = mdb.models['Model-1'].parts['Element_Vor']
